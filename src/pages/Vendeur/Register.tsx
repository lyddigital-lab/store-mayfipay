...
export default function VendeurRegister() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (existing) {
        setError('Un compte existe déjà avec ce numéro. Connectez-vous.');
        setLoading(false);
        return;
      }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-white rounded-2xl shadow-lg mb-3">
            <img src="/logo.png" alt="MayfiPay" className="h-10 w-10 object-contain" onError={(e) => (e.currentTarget.style.display = 'none')} />
          </div>
          <h1 className="text-2xl font-bold text-white">MayfiPay</h1>
          <p className="text-white/80 text-sm mt-1">Créer un compte vendeur</p>
        </div>

        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-xl">
          <h2 className="text-lg font-bold text-gray-900 mb-1">Inscription</h2>
          <p className="text-sm text-gray-500 mb-5">Créez votre compte MayfiPay vendeur</p>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading || !nom || !phone || !password}
          ...
          <p className="text-center text-sm text-gray-500 mt-5">
            Déjà un compte ?{' '}
            <Link to="/vendeur/login" className="text-orange-500 font-semibold hover:underline">
              Se connecter
            </Link>
          </p>
        </div>
